...
public class PostController extends Controller {
	...
	public Response postMessage(PostResource post, String userId) throws SQLException {
		...
		if (post.getContent() == null) {
			...
			this.getBadRequestResponse(res, "Unable to post message: " + "Content is of the post is empty.");
		}
	...
	public Response deleteMessage(String userId, String postId) throws SQLException {
		...
		if (affectedRows > 0) {
			// Prepare data to send back to client
			return this.getOkResponse(res, post, "Post deleted succesfully");
		}
	...
	public Response getMessage(String userId, String limitTo) throws SQLException {
		...
		if (rs.next()) {
			// Prepare data to send back to client
			post.setlocation(this.getPath() + "/post/" + rs.getString("post_id"));
	...
	public Response getMessageFriends(String userId, String content) throws SQLException {
		...
		if (rs != null) {
			...
			while (rs.next()) {
				posts.add(new PostResource(rs.getString("post_id"), rs.getString("user_id"), rs.getString("content"),
						rs.getString("created_at"), "http://localhost:8080/SOS/api/user/" + rs.getString("user_id"),
