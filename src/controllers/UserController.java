...
public class UserController extends Controller {
	...
	public Response createUser(UserResource user) throws SQLException {
		...
		if (user.getName() == null || user.getUsername() == null) {
			return this.getBadRequestResponse(res, "Unable to create user. "
					+ "One or more of the following fields are empty: " + "user or/and username");
		}
	...
	public Response editUser(String userId, UserResource user) throws SQLException {
		...
		if (rowsAffected > 0) {
			// Prepare data to send back to client
			UserResource fullUserInfo = (UserResource) ((HashMap<String, Object>) userInformation.getEntity())
					.get("data");
	...
	public Response getUsers(String name, int limitTo, int page) throws SQLException {
		...
		if (rs != null) {
			...
			while (rs.next()) {
				UserResource user = new UserResource(rs.getInt("user_id"), rs.getString("name"),
						rs.getString("username"), rs.getString("email"), rs.getString("biography"), this.getPath(),
	...
	public Response getFriends(String userId, String name, int limitTo, int page) throws SQLException {
		...
		if (rs != null) {
			...
			while (rs.next()) {
				friends.add(new UserResource(rs.getString("UserID2"), rs.getString("name"), rs.getString("username"),
						// Set location
